  ...
  try {
    ...
    const [stats] = await db.query(
      `SELECT
        ...
        COUNT(*) AS total
      FROM asistencias WHERE fecha = ?`, [hoy]);
...
exports.faltasCriticas = async (req, res, next) => {
  try {
    ...
    const rows = await db.query(
      ...
      WHERE a.activo = 1
      GROUP BY a.id HAVING faltas >= ? ORDER BY faltas DESC`,
...
exports.porGrupo = async (req, res, next) => {
  try {
    ...
    const rows = await db.query(
      ...
      WHERE g.activo = 1 GROUP BY g.id ORDER BY g.nombre`,
      [desde, hasta]);
...
exports.retardosCriticos = async (req, res, next) => {
  try {
    ...
    const rows = await db.query(
      ...
      LEFT JOIN asistencias as2 ON as2.alumno_id = a.id AND as2.fecha >= ?
      WHERE a.activo = 1
      GROUP BY a.id HAVING retardos >= ? ORDER BY retardos DESC LIMIT 10`,
      [desde, parseInt(umbral)]);
    res.json({ ok: true, datos: rows, parametros: { umbral, dias, desde } });
  } catch (e) { next(e); }
};
...
exports.sugerenciasIA = async (req, res, next) => {
  try {
    ...
    const [resumen, porGrupo, faltasCriticas, tendenciaMensual, porMateria] = await Promise.all([
      ...
      db.query(`SELECT g.nombre AS grupo, COUNT(CASE WHEN as2.tipo='asistencia' THEN 1 END) AS asistencias, COUNT(CASE WHEN as2.tipo='retardo' THEN 1 END) AS retardos, COUNT(CASE WHEN as2.tipo='falta' THEN 1 END) AS faltas FROM grupos g LEFT JOIN alumnos a ON a.grupo_id=g.id AND a.activo=1 LEFT JOIN asistencias as2 ON as2.alumno_id=a.id AND as2.fecha BETWEEN ? AND ? WHERE g.activo=1 GROUP BY g.id`, [desde, hasta]),
      db.query(`SELECT CONCAT(a.nombre,' ',a.apellido_pat) AS alumno, g.nombre AS grupo, COUNT(CASE WHEN as2.tipo='falta' THEN 1 END) AS faltas FROM alumnos a JOIN grupos g ON g.id=a.grupo_id LEFT JOIN asistencias as2 ON as2.alumno_id=a.id AND as2.fecha >= ? WHERE a.activo=1 GROUP BY a.id HAVING faltas >= 3 ORDER BY faltas DESC LIMIT 10`, [desde]),
      db.query(`SELECT DATE_FORMAT(fecha,'%Y-%m') AS mes, COUNT(CASE WHEN tipo='asistencia' THEN 1 END) AS asistencias, COUNT(CASE WHEN tipo='retardo' THEN 1 END) AS retardos, COUNT(CASE WHEN tipo='falta' THEN 1 END) AS faltas FROM asistencias WHERE fecha >= DATE_SUB(CURDATE(), INTERVAL 6 MONTH) GROUP BY mes ORDER BY mes`),
...
// ================================================================
// HELPER: obtener todos los datos para exportación
// ================================================================
async function obtenerDatosReporte(query = {}) {
  ...
  const [resumen, porGrupo, faltasCriticas, tendenciaMensual, porMateria] = await Promise.all([
    db.query(`SELECT COUNT(CASE WHEN tipo='asistencia' THEN 1 END) AS asistencias, COUNT(CASE WHEN tipo='retardo' THEN 1 END) AS retardos, COUNT(CASE WHEN tipo='falta' THEN 1 END) AS faltas, COUNT(CASE WHEN tipo='permiso' THEN 1 END) AS permisos, COUNT(*) AS total FROM asistencias WHERE fecha BETWEEN ? AND ?`, [desde, hasta]),
    db.query(`SELECT g.nombre AS grupo, COUNT(CASE WHEN as2.tipo='asistencia' THEN 1 END) AS asistencias, COUNT(CASE WHEN as2.tipo='retardo' THEN 1 END) AS retardos, COUNT(CASE WHEN as2.tipo='falta' THEN 1 END) AS faltas, COUNT(CASE WHEN as2.tipo='permiso' THEN 1 END) AS permisos FROM grupos g LEFT JOIN alumnos a ON a.grupo_id=g.id AND a.activo=1 LEFT JOIN asistencias as2 ON as2.alumno_id=a.id AND as2.fecha BETWEEN ? AND ? WHERE g.activo=1 GROUP BY g.id ORDER BY g.nombre`, [desde, hasta]),
    db.query(`SELECT a.matricula, CONCAT(a.nombre,' ',a.apellido_pat) AS alumno, g.nombre AS grupo, COUNT(CASE WHEN as2.tipo='falta' THEN 1 END) AS faltas, COUNT(CASE WHEN as2.tipo='retardo' THEN 1 END) AS retardos, COUNT(CASE WHEN as2.tipo='permiso' THEN 1 END) AS permisos FROM alumnos a JOIN grupos g ON g.id=a.grupo_id LEFT JOIN asistencias as2 ON as2.alumno_id=a.id AND as2.fecha >= ? WHERE a.activo=1 GROUP BY a.id HAVING faltas >= ? ORDER BY faltas DESC`, [desde, parseInt(umbral_faltas)]),
    db.query(`SELECT DATE_FORMAT(fecha,'%Y-%m') AS mes, COUNT(CASE WHEN tipo='asistencia' THEN 1 END) AS asistencias, COUNT(CASE WHEN tipo='retardo' THEN 1 END) AS retardos, COUNT(CASE WHEN tipo='falta' THEN 1 END) AS faltas FROM asistencias WHERE fecha >= DATE_SUB(CURDATE(), INTERVAL 6 MONTH) GROUP BY mes ORDER BY mes`),
    db.query(`SELECT m.nombre AS materia, COUNT(CASE WHEN a.tipo='falta' THEN 1 END) AS faltas FROM asistencias a JOIN horarios h ON h.id=a.horario_id JOIN materias m ON m.id=h.materia_id WHERE a.fecha BETWEEN ? AND ? GROUP BY m.id ORDER BY faltas DESC LIMIT 5`, [desde, hasta])
  ]);
...
exports.exportarExcel = async (req, res, next) => {
  try {
    ...
    const { desde, hasta, resumen, porGrupo, faltasCriticas, tendenciaMensual, sugerencias } = datos;
    ...
    wsResumen.columns = [{ header: 'Concepto', key: 'concepto', width: 28 }, { header: 'Cantidad', key: 'cantidad', width: 14 }, { header: 'Porcentaje', key: 'pct', width: 14 }];
    wsResumen.mergeCells('A1:C1'); const tR = wsResumen.getCell('A1');
    tR.value = `SIGA — Reporte de Asistencias  (${desde} al ${hasta})`;
    tR.font = { bold: true, size: 13, color: { argb: 'FF1a6e4a' } }; tR.alignment = { horizontal: 'center' };
    wsResumen.getRow(1).height = 26; wsResumen.addRow([]);
    aplicarEncabezado(wsResumen.addRow(['Concepto', 'Cantidad', 'Porcentaje']));
    const total = resumen.total || 1;
    [['✅ Asistencias', resumen.asistencias, resumen.asistencias/total], ['⚠  Retardos', resumen.retardos, resumen.retardos/total], ['🚨 Faltas', resumen.faltas, resumen.faltas/total], ['📋 Permisos', resumen.permisos, resumen.permisos/total], ['TOTAL', resumen.total, 1]]
      .forEach(([concepto, cantidad, pct]) => {
        const row = wsResumen.addRow({ concepto, cantidad, pct });
        row.getCell('cantidad').style = estiloNumero; row.getCell('pct').style = estiloPct;
        if (concepto === 'TOTAL') { row.font = { bold: true }; row.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFf0fdf4' } }; }
      });

    // HOJA 2 — Por grupo
    const wsGrupos = wb.addWorksheet('Por Grupo');
    wsGrupos.columns = [{ header: 'Grupo', key: 'grupo', width: 24 }, { header: 'Asistencias', key: 'asistencias', width: 14 }, { header: 'Retardos', key: 'retardos', width: 12 }, { header: 'Faltas', key: 'faltas', width: 10 }, { header: 'Permisos', key: 'permisos', width: 11 }, { header: 'Total', key: 'tot', width: 10 }, { header: '% Asistencia', key: 'pct', width: 14 }];
    wsGrupos.mergeCells('A1:G1'); const tG = wsGrupos.getCell('A1');
    tG.value = `Asistencia por Grupo  (${desde} al ${hasta})`; tG.font = { bold: true, size: 12, color: { argb: 'FF1a6e4a' } }; tG.alignment = { horizontal: 'center' };
    wsGrupos.getRow(1).height = 24; wsGrupos.addRow([]);
    aplicarEncabezado(wsGrupos.addRow(['Grupo', 'Asistencias', 'Retardos', 'Faltas', 'Permisos', 'Total', '% Asistencia']));
    porGrupo.forEach(r => {
    ...
    wsFaltas.columns = [{ header: 'Matrícula', key: 'matricula', width: 14 }, { header: 'Alumno', key: 'alumno', width: 30 }, { header: 'Grupo', key: 'grupo', width: 20 }, { header: 'Faltas', key: 'faltas', width: 10 }, { header: 'Retardos', key: 'retardos', width: 11 }, { header: 'Permisos', key: 'permisos', width: 11 }];
    wsFaltas.mergeCells('A1:F1'); const tF = wsFaltas.getCell('A1');
    tF.value = `Alumnos con Faltas Críticas  (últimos ${req.query.dias||30} días, umbral ≥ ${req.query.umbral_faltas||3})`;
    tF.font = { bold: true, size: 12, color: { argb: 'FFb91c1c' } }; tF.alignment = { horizontal: 'center' };
    wsFaltas.getRow(1).height = 24; wsFaltas.addRow([]);
    aplicarEncabezado(wsFaltas.addRow(['Matrícula', 'Alumno', 'Grupo', 'Faltas', 'Retardos', 'Permisos']));
    if (!faltasCriticas.length) { wsFaltas.addRow(['—', 'Sin alumnos en esta condición', '', 0, 0, 0]); }
    else faltasCriticas.forEach(r => {
    ...
    wsTend.columns = [{ header: 'Mes', key: 'mes', width: 14 }, { header: 'Asistencias', key: 'asistencias', width: 14 }, { header: 'Retardos', key: 'retardos', width: 12 }, { header: 'Faltas', key: 'faltas', width: 10 }, { header: 'Total', key: 'tot', width: 10 }, { header: '% Asistencia', key: 'pct', width: 14 }];
    wsTend.mergeCells('A1:F1'); const tT = wsTend.getCell('A1');
    tT.value = 'Tendencia Mensual (últimos 6 meses)'; tT.font = { bold: true, size: 12, color: { argb: 'FF1a6e4a' } }; tT.alignment = { horizontal: 'center' };
    wsTend.getRow(1).height = 24; wsTend.addRow([]);
    ...
    // HOJA 5 — ✨ Sugerencias IA
    const wsSug = wb.addWorksheet('Sugerencias IA');
    wsSug.columns = [{ header: 'N°', key: 'num', width: 6 }, { header: 'Sugerencia generada por Gemini AI', key: 'sug', width: 90 }];
    wsSug.mergeCells('A1:B1'); const tS = wsSug.getCell('A1');
    tS.value = '✨ Sugerencias de Inteligencia Artificial — SIGA (Powered by Gemini)';
    tS.font = { bold: true, size: 13, color: { argb: 'FF4f46e5' } }; tS.alignment = { horizontal: 'center' };
    wsSug.getRow(1).height = 28;
    wsSug.mergeCells('A2:B2'); const tS2 = wsSug.getCell('A2');
    tS2.value = `Generadas el ${new Date().toLocaleString('es-MX')} · Período analizado: ${desde} al ${hasta}`;
    tS2.font = { italic: true, color: { argb: 'FF6b7280' }, size: 10 }; tS2.alignment = { horizontal: 'center' };
    wsSug.addRow([]);
    aplicarEncabezado(wsSug.addRow(['N°', 'Sugerencia']));
    if (!sugerencias.length) {
      wsSug.addRow([1, 'No se pudieron generar sugerencias (verifica la clave GEMINI_API_KEY)']);
    } else {
      sugerencias.forEach((s, i) => {
        const row = wsSug.addRow({ num: i + 1, sug: s });
        row.getCell('sug').alignment = { wrapText: true };
        row.height = 32;
        if (i % 2 === 0) row.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFf5f3ff' } };
      });
    }
...
exports.exportarPDF = async (req, res, next) => {
  ...
  try {
    ...
    const filasSugerencias = sugerencias.length
      ? sugerencias.map((s, i) => `<div class="sug-item"><span class="sug-num">${i+1}</span><span>${s}</span></div>`).join('')
      : `<p style="color:#6b7280;font-style:italic">No se pudieron generar sugerencias automáticas.</p>`;
...
.ai-section { background:linear-gradient(135deg,#f5f3ff 0%,#ede9fe 100%); border:1px solid #c4b5fd; border-radius:8px; padding:16px; margin-top:20px; }
.ai-section h2 { border-bottom:1px solid #c4b5fd; color:#4f46e5; margin-top:0; }
.ai-badge { display:inline-block; background:#4f46e5; color:#fff; font-size:9px; padding:2px 7px; border-radius:10px; margin-left:8px; vertical-align:middle; }
.sug-item { display:flex; gap:10px; align-items:flex-start; padding:7px 0; border-bottom:1px solid #e9d5ff; }
.sug-item:last-child { border-bottom:none; }
.sug-num { background:#4f46e5; color:#fff; border-radius:50%; width:18px; height:18px; display:flex; align-items:center; justify-content:center; font-size:9px; font-weight:700; flex-shrink:0; margin-top:1px; }
...
<div class="ai-section">
  <h2>✨ Sugerencias de Inteligencia Artificial <span class="ai-badge">Powered by Gemini</span></h2>
  ${filasSugerencias}
</div>
