...
function origenBadge(origen) {
  // "sensor" usa info, "manual" usa el badge sin clase (gris por defecto)
...
function formatFecha(fechaStr) {
  if (!fechaStr) return "-";
  // fechaStr viene como "YYYY-MM-DD" desde MySQL
...
async function buscar() {
  ...
  // Validar rango de fechas antes de llamar al servidor
...
function renderResumen() {
  ...
  const partes = [];
  if (conteo.asistencia)
    partes.push(
      `<span class="badge ok">${conteo.asistencia} asistencia${conteo.asistencia !== 1 ? "s" : ""}</span>`,
    );
  if (conteo.retardo)
    partes.push(
      `<span class="badge warn">${conteo.retardo} retardo${conteo.retardo !== 1 ? "s" : ""}</span>`,
    );
  if (conteo.falta)
    partes.push(
      `<span class="badge no">${conteo.falta} falta${conteo.falta !== 1 ? "s" : ""}</span>`,
    );
  if (conteo.permiso)
    partes.push(
      `<span class="badge info">${conteo.permiso} permiso${conteo.permiso !== 1 ? "s" : ""}</span>`,
    );
...
function renderPagina() {
  ...
  tablaHistorial.innerHTML = slice
    .map(
      (a) => `
    <tr>
  ...
  if (totalPags > 1) {
    infoPagina.textContent = `Página ${paginaActual} de ${totalPags}`;
...
btnSiguiente.addEventListener("click", () => {
  if (paginaActual < Math.ceil(todosLosDatos.length / POR_PAGINA)) {
    paginaActual++;
    renderPagina();
    window.scrollTo({ top: 0, behavior: "smooth" });
  }
...
// Enter en fechas dispara la búsqueda
