// ================================================================
// SIGA — Módulo 5: Dashboard con gráficas Chart.js
// ================================================================
...
async function cargarResumenHoy() {
  try {
    ...
    console.error("Error KPIs hoy:", e);
  }
}
...
async function cargarResumenMes() {
  try {
    ...
    console.error("Error resumen mes:", e);
  }
}
...
async function cargarChartDona() {
  try {
    ...
    chartDona = new Chart(ctx, {
      ...
      options: {
        responsive: true,
        // FIX: mantener ratio cuadrado para que no se distorsione
    ...
    console.error("Error chart dona:", e);
  }
}
...
async function cargarChartLinea() {
  try {
    ...
    console.error("Error chart línea:", e);
  }
}
...
async function cargarChartGrupos() {
  try {
    ...
    console.error("Error chart grupos:", e);
  }
}
...
async function cargarChartMaterias() {
  try {
    ...
    console.error("Error chart materias:", e);
  }
}
...
async function cargarFaltasCriticas() {
  try {
    const res = await SIGA.api(
      "/api/reportes/faltas-criticas?umbral=1&dias=30",
    );
    const rows = (res.datos || []).slice(0, 8);
    if (!rows.length) {
      tablaFaltas.innerHTML =
        '<p class="muted-label">Sin faltas registradas.</p>';
      return;
    }
    tablaFaltas.innerHTML = `
      <table class="tabla-criticos">
        <thead><tr><th>Alumno</th><th>Grupo</th><th style="text-align:right;">Faltas</th></tr></thead>
        <tbody>${rows
          .map(
            (r) => `
          <tr>
...
async function cargarRetardosCriticos() {
  try {
    const res = await SIGA.api(
      "/api/reportes/retardos-criticos?umbral=1&dias=30",
    );
    const rows = (res.datos || []).slice(0, 8);
    if (!rows.length) {
      tablaRetardos.innerHTML =
        '<p class="muted-label">Sin retardos registrados.</p>';
      return;
    }
    tablaRetardos.innerHTML = `
      <table class="tabla-criticos">
        <thead><tr><th>Alumno</th><th>Grupo</th><th style="text-align:right;">Retardos</th></tr></thead>
        <tbody>${rows
          .map(
            (r) => `
          <tr>
    ...
    console.error("Error retardos:", e);
  }
}
...
function renderFeed() {
  if (!feedItems.length) {
    feedAsistencias.innerHTML =
      '<p class="muted-label" style="padding:12px 0;">Sin asistencias hoy.</p>';
    feedContador.textContent = "";
    return;
  }
  feedContador.textContent = `${feedItems.length} hoy`;
  feedAsistencias.innerHTML = feedItems
    .map(
      (a) => `
    <div class="feed-item">
      <span class="feed-hora">${a.hora_entrada ? a.hora_entrada.slice(0, 5) : "--:--"}</span>
      <span class="feed-nombre">${a.nombre} <span style="color:var(--muted);font-size:12px;">${a.matricula}</span></span>
...
async function cargarTodo() {
  await Promise.all([
    ...
    cargarChartGrupos(),
    cargarChartMaterias(),
    cargarFaltasCriticas(),
    cargarRetardosCriticos(),
    cargarFeedInicial(),
  ]);
}
